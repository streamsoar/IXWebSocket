...
namespace ix
{
        ...
        int timeoutSecs)
    {
        ...
        if (httpVersion != "HTTP/1.1")
        {
            ...
            ss2 << "Expecting HTTP/1.1, got " << httpVersion << ". "
               << "Rejecting connection to " << url << ", status: " << status
               << ", HTTP Status line: " << line;
            return WebSocketInitResult(false, status, ss2.str());
        }
        ...
        {
            std::stringstream ss2;
            ss2 << "Expecting status 101 (Switching Protocol), got " << status
               << " status connecting to " << url << ", HTTP Status line: " << line;

            return WebSocketInitResult(false, status, ss2.str(), headers, path);
        }
        ...
        {
            std::stringstream ss2;
            ss2 << "Invalid connection value: " << headers["connection"];
            return WebSocketInitResult(false, status, ss2.str());
        }
